...
export const initCart = async () => {
  ...
  // Обробка подій зміни кількості товару та видалення товару з кошика
  ...
  // Обробка подій модального вікна кошика
  ...
  // Ініціалізація подій для форми оформлення замовлення
  ...
  if (list && data.basket_items) {
    ...
    if (badgeEl) {
      ...
      document
        ...
        .insertAdjacentHTML('beforeend', `<span id="cart-modal-button-badge" class="badge success">${data.basket_items.length}</span>`);
    }
    initCart();
  }
...
async function changeBasketTotal(cartData) {
  ...
  if (data.basket_sum !== void 0) {
    ...
    //   totalToPay.deliveryCost = totalToPay.deliveryCost + smallOrderDeliveryPrice;
    //   document.querySelector('sl-alert[data-name="small-order"]').show();
    // } else {
    //   document.querySelector('sl-alert[data-name="small-order"]').hide();
    //   totalToPay.deliveryCost = totalToPay.deliveryCost - smallOrderDeliveryPrice;
...
function refreshTotal(totalToPay) {
  ...
  document.getElementById('input-hidden-basket_totalprice').value = Number(totalToPay.productPrice);
  document.getElementById('input-hidden-cost_delivery').value = Number(totalToPay.deliveryCost);
  document.getElementById('input-hidden-to_pay').value = Number(totalToPay.productPrice + totalToPay.deliveryCost);
}
